...
async def remove_admin(user_id: str) -> bool:
    ...
    async with db_pool.acquire() as conn:
        ...
        if str(result).startswith("DELETE 1"):
...
async def addadmin(ctx, member: discord.Member = None):
    if str(ctx.author.id) != str(bot_owner_id): return await cute_refuse(ctx, "add admins")
...
async def removeadmin(ctx, member: discord.Member = None):
    if str(ctx.author.id) != str(bot_owner_id): return await cute_refuse(ctx, "remove admins")
