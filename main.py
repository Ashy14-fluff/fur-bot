...
async def remove_admin(user_id: str) -> bool:
    ...
    async with db_pool.acquire() as conn:
        ...
        # Fixed: Use proper asyncpg result check
        if result == "DELETE 1":
...
async def ask_ai(messages: List[dict]) -> str:
    def call_groq():
        try:
            ...
            return "mrrp... me got too excited an' mah bwain got too fuww of fwuff... sowwy 🥺"
...
async def nsfw(ctx, mode: str = "on"):
    ...
    if mode == "on":
        ...
        await ctx.send("use `!nsfw on` / `!nsfw off`~")


@bot.command()
async def lewd(ctx, level:
